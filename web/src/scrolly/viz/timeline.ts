...
export default function renderTimeline({ mountEl, legendEl, props }: TimelineArgs) {
  ...
  // Compute y-domain: use props.yDomain if given, else auto-scale from data
  const allValues = data.flatMap((d: any) =>
    (props?.series ?? ["internet", "tv", "radio"]).map((k: string) => Number(d[k]) || 0)
  );
  const autoMax = d3.max(allValues) ?? 100;
  const yDomain = (props?.yDomain ?? [0, Math.ceil(autoMax * 1.15)]) as [number, number];
  ...
  // props.seriesLabels: Record<string, string> — display name per key; falls back to capitalised key
  const seriesLabels = (props?.seriesLabels || {}) as Record<string, string>;
  ...
  series.forEach((key) => {
    ...
    svg
      ...
      .text(seriesLabels[key] ?? (key.charAt(0).toUpperCase() + key.slice(1)))
      .attr("opacity", 0)
      .transition()
      .delay(1800)
      .attr("opacity", 1);
