...
export default function renderAccuracy({ mountEl, props }: AccuracyArgs) {
    ...
    // Variables — configurable via props, fallback to original Indonesia election demo
    const total: number = props?.total ?? 34;
    const correct: number = props?.correct ?? 13;
    ...
    svg.append("text")
        ...
        .text(props?.sublabel ?? `out of ${total} provinces correctly predicted`);
    ...
    const incorrectLabel = svg.append("text")
        ...
        .style("opacity", 0)
        .text(props?.incorrectLabel ?? `${incorrect} Incorrect`);
    ...
    const correctLabel = svg.append("text")
        ...
        .style("font-weight", "600")
        .style("opacity", 0)
        .text(props?.correctLabel ?? `${correct} Correct`);
